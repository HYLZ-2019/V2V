...
from .image_utils import ImagePadder, forward_interpolate_pytorch
...
class ERAFT(nn.Module):
    def __init__(self, config, n_first_channels):
        ...
        self.fnet = BasicEncoder(output_dim=256, norm_fn='instance', dropout=0, n_first_channels=n_first_channels)
        self.cnet = BasicEncoder(output_dim=hdim+cdim, norm_fn='batch', dropout=0, n_first_channels=n_first_channels)
    ...
    def forward_(self, image1, image2, iters=12):
    ...
    # Wrap in forward code from E-RAFT/test.py/TestRaftEventsWarm.run_network
    def forward(self, image1, image2, iters=12):
        flow_low_res, flow_list = self.forward_(image1=image1, image2=image2, iters=iters)
        # Keep results for next recurrent call
        self.flow_init = forward_interpolate_pytorch(flow_low_res)
        flow_est = flow_list[-1]
        return flow_est
