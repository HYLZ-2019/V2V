...
def temporal_consistency_loss(image0, image1, processed0, processed1, flow01, alpha=50.0, output_images=False, reduce_batch=True):
    ...
    """
    #print("Max and min of processed0: ", processed0.min().item(), processed0.max().item())
    t_width, t_height = image0.shape[3], image0.shape[2]
    xx, yy = torch.meshgrid(
        torch.arange(t_width, device=image0.device),
        torch.arange(t_height, device=image0.device),
        indexing="xy")  # xx, yy -> WxH
    ...
    # Add clamp, to prevent loss hack (predict processed to +inf -> loss becomes 0)
    processed0 = torch.clamp(processed0, 0, 255)
    processed0_warped_to1 = F.grid_sample(processed0, warping_grid, align_corners=True)
    #print("Mean of visibility_mask: ", torch.mean(visibility_mask).item())

    div = torch.abs(processed1) + torch.abs(processed0_warped_to1) + 1e-5

    tc_map = visibility_mask * torch.abs(processed1 - processed0_warped_to1) / div

    if reduce_batch:
        tc_loss = tc_map.mean()
    else:
        tc_loss = tc_map.mean(dim=(1,2,3))
