...
def events_to_voxel_torch(xs, ys, ts, ps, B, device=None, sensor_size=(180, 240), temporal_bilinear=True):
    ...
    if temporal_bilinear:
        ...
        bins = torch.zeros((B, sensor_size[0], sensor_size[1]), dtype=torch.float32)
        t_per_bin = (dt + 0.001) / B
        bin_idx = torch.floor((ts - ts[0]) / t_per_bin).to(dtype=torch.int)
        bins.index_put_((bin_idx, ys.to(dtype=torch.int), xs.to(dtype=torch.int)), ps, accumulate=True)
